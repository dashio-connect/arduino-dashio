...
void processStatus(ConnectionType connectionType) {
    ...
    message += dashDevice.getChartLineInts(CHART_ID, "L1", "Line 1", peakBar, "purple", data, sizeof(data)/sizeof(int));
    sendMessage(connectionType, message);
    
    int data2[] = {100, 200, 300, 250, 225, 280, 350, 500, 550, 525, 500};
    message = dashDevice.getChartLineInts(CHART_ID, "L2", "Line 2", line, "aqua", data2, sizeof(data2)/sizeof(int));
    sendMessage(connectionType, message);
}
...
void processButton(MessageData *messageData) {
    if (messageData->idStr == BUTTON_ID) {
        if (toggle == off) {
            ...
            String message = dashDevice.getChartLineInts(CHART_ID, "L1", "Line 1 a", bar, "green", data, sizeof(data)/sizeof(int));
            int data2[] = {153, 351, 806, 900, 200, 0};
            message += dashDevice.getChartLineInts(CHART_ID, "L2", "Line 2 a", segBar, "yellow", data2, sizeof(data2)/sizeof(int));
            sendMessage(messageData->connectionType, message);
        } else if (toggle == on) {
            toggle = off;
            float data[] = {200, 303.334, 504.433, 809.434, 912, 706, 643};
            String message = dashDevice.getChartLineFloats(CHART_ID, "L1", "Line 1 b", peakBar, "orange", data, sizeof(data)/sizeof(float));
            int data2[] = {};
            message += dashDevice.getChartLineInts(CHART_ID, "L2", "Line 2 b", peakBar, "blue", data2, sizeof(data2)/sizeof(int));
            sendMessage(messageData->connectionType, message);
        }
